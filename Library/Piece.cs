...
namespace Library
{
    ...
    public abstract class Piece : PropertyChangedBase, ICloneable
    {
        ...
        public abstract bool CanBeMovedToSquare(Square square);//можно ли её переместить тудато

        public bool CheckCollision(Square end, Board board)
        {
            var dir = ChooseRightDirection(end.Point);//может ли переместится
            if (dir == null)
                return false; 
        ...
        public Point ChooseRightDirection(Point end)//направление двежения
        {
            foreach (var dir in Directions)
                if (Point.IsInDirection(end, dir))//блокирует ли
                    return dir;
