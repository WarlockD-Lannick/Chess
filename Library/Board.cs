...
namespace Library
{
    public class Board : PropertyChangedBase, ICloneable
    {
        ...
        public Piece IsKingChecked(Color turn)              //проверка шаха
        {
        ...
        public bool IsPieceBlocking(Piece piece, Square end, Point dir)         //проверка блока
        {
        ...
        public Color GetOtherColor(Color color)//проверка на цвет фигур
        {
        ...
        public List<Piece> GetAllPiecesByColor(Color color)                 //одного цвета
        {
        ...
        public List<Piece> GetAllPieces()               //все 
        {
        ...
        //Перемещает фигуру с одного квадрата на другой и возвращает фигуру, которая была на целевой клетке (если такая была)
        {
        ...
        public List<Move> CalcPossibleMoves(Piece piece)
        {
            ...
            foreach (var dir in piece.Directions)       //коллеуция передвежения фигур
            {
                var allMoves = piece.Point.AllMovesWithinDirection(dir);           //возможные клетки
                foreach (var end in allMoves)//проверка
                {
                    var square = Squares.FirstOrDefault(x => x.Point.Equals(end)); //кл.на доске
                    if (piece.CanMoveWithoutColliding(square, this))//пр.на столкновение
                    {
                        var clonedBoard = PredictBoard(piece, square);
                        if (clonedBoard.IsKingChecked(piece.Color) == null)//пр.на шах
                        {
                            var add = CanCastle(piece);//ракир
                            foreach (var castle in add)
                            {
                                if (!piece.CheckCollision(castle[0], this)) {//блок
                                    var cMove = new Castle(this, piece, castle[0], castle[1]);//объект
                                    cMove.OnInitiatePawnPromotion += Move_OnInitiatePawnPromotion;//событие
                                    cMove.OnPieceCaptured += Move_OnPieceCaptured;
                                    res.Add(cMove);
                                }                                
                            }

                            var move = new Move(this, piece, square);//обыч.ход
        ...
        public List<Square[]> CanCastle(Piece start)//метод ракировки
        {
